...
class Pixel {
  ...
  constructor(canvas: HTMLCanvasElement, context: CanvasRenderingContext2D, x: number, y: number, color: string, speed: number, delay: number) {
    this.width = canvas.width; this.height = canvas.height; this.ctx = context; this.x = x; this.y = y; this.color = color;
    this.speed = this.getRandomValue(0.1, 0.9) * speed; this.size = 0; this.sizeStep = Math.random() * 0.4; this.minSize = 0.5;
    this.maxSizeInteger = 2; this.maxSize = this.getRandomValue(this.minSize, this.maxSizeInteger); this.delay = delay;
    this.counter = 0; this.counterStep = Math.random() * 4 + (this.width + this.height) * 0.01;
...
// Factory — only called in the browser so HTMLElement is guaranteed to exist
function createPixelCanvasClass() {
  return class PixelCanvasElement extends HTMLElement {
    ...
    get speed() { const v = Number(this.dataset.speed) || 35; return this.reducedMotion ? 0 : Math.max(0, Math.min(100, v)) * 0.001; }
    ...
    connectedCallback() {
      ...
      requestAnimationFrame(() => {
        this.handleResize();
        const ro = new ResizeObserver((entries) => { if (!entries.length) return; requestAnimationFrame(() => this.handleResize()); });
        ro.observe(this);
    ...
    getDistanceToCenter(x: number, y: number) { const dx = x - this.canvas.width / 2; const dy = y - this.canvas.height / 2; return Math.sqrt(dx * dx + dy * dy); }
    getDistanceToBottomLeft(x: number, y: number) { const dx = x; const dy = this.canvas.height - y; return Math.sqrt(dx * dx + dy * dy); }
    ...
    createPixels() {
      ...
      for (let x = 0; x < this.canvas.width; x += this.gap) {
        for (let y = 0; y < this.canvas.height; y += this.gap) {
          ...
          const delay = this.reducedMotion ? 0 : (this.variant === "icon" ? this.getDistanceToCenter(x, y) : this.getDistanceToBottomLeft(x, y));
    ...
    handleAnimation(name: "appear" | "disappear") {
      ...
      const animate = () => {
        ...
        const timeNow = performance.now(); const timePassed = timeNow - this.timePrevious;
...
export interface PixelCanvasProps extends React.HTMLAttributes<HTMLDivElement> {
  gap?: number; speed?: number; colors?: string[]; variant?: "default" | "icon"; noFocus?: boolean;
}
