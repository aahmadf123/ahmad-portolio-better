...
function usePreviewSource(url: string, width: number, height: number, quality: number, isStatic: boolean, staticImageSrc?: string) {
  return useMemo(() => {
    ...
    const params = encode({
      url, screenshot: true, meta: false, embed: "screenshot.url", colorScheme: "dark",
      "viewport.isMobile": true, "viewport.deviceScaleFactor": 1,
...
export const HoverPeek = ({
  children, url, className, peekWidth = 200, peekHeight = 125, imageQuality = 50,
  isStatic = false, imageSrc = "", enableMouseFollow = true, enableLensEffect = true,
  lensZoomFactor = 1.75, lensSize = 100,
}: HoverPeekProps) => {
  const [imageLoadFailed, setImageLoadFailed] = useState(false);
  const finalImageSrc = usePreviewSource(url, peekWidth, peekHeight, imageQuality, isStatic, imageSrc);
  ...
  useEffect(() => { if (!isPeeking) { setImageLoadFailed(false); setIsHoveringLens(false); } }, [isPeeking]);
  ...
  const handleLensMouseEnter = () => { if (enableLensEffect) setIsHoveringLens(true); };
  const handleLensMouseLeave = () => { if (enableLensEffect) setIsHoveringLens(false); };
  ...
  const lensMotionVariants = {
    ...
    animate: { opacity: 1, scale: 1, transition: { duration: 0.2, ease: [0.0, 0.0, 0.2, 1] as [number, number, number, number] } },
    exit: { opacity: 0, scale: 0.7, transition: { duration: 0.2, ease: [0.4, 0.0, 1.0, 1] as [number, number, number, number] } },
  };
    ...
    <RdxHoverCard.Root openDelay={75} closeDelay={150} onOpenChange={handleOpenChange}>
      ...
      <RdxHoverCard.Portal>
        <RdxHoverCard.Content
          ...
          side="top" align="center" sideOffset={12}
            ...
            {isPeeking && (
              ...
              >
                <a href={url} target="_blank" rel="noopener noreferrer"
                  className={cn("relative block overflow-hidden rounded-lg bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700 shadow-lg hover:shadow-xl transition-shadow p-0.5")}
                  onMouseEnter={handleLensMouseEnter} onMouseLeave={handleLensMouseLeave} onMouseMove={handleLensMouseMove}
                >
                  {imageLoadFailed ? (
                    <div className="flex items-center justify-center bg-neutral-100 dark:bg-neutral-800 text-neutral-500 dark:text-neutral-400 text-xs font-sans" style={{ width: peekWidth, height: peekHeight }}>
                      Preview unavailable
                    </div>
                  ) : (
                    <img src={finalImageSrc} width={peekWidth} height={peekHeight}
                      ...
                      alt={`Link preview for ${url}`} onError={() => setImageLoadFailed(true)} loading="lazy"
                    />
                  )}
                  <AnimatePresence>
